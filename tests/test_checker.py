...
def test_classify_returns_none_when_all_classifiers_return_none(mock_get_classifiers):
...
@patch('flake8_digit_separator.checker.ClassifierRegistry.get_ordered_classifiers')
def test_classify_returns_none_when_all_classifiers_return_falsy(mock_get_classifiers):
    ...
    mock_instance3 = Mock()
    mock_instance3.classify.return_value = 0
...
@patch('flake8_digit_separator.checker.ClassifierRegistry.get_ordered_classifiers')
def test_classify_returns_none_when_no_classifiers(mock_get_classifiers):
...
def test_classify_returns_number_when_classifier_finds_match():
    """Test that _classify returns the number when a classifier finds a match."""
    mock_token = Mock()
    mock_token.string = '123'
...
def test_process_number_token_returns_none_when_classify_returns_none(mock_classify):
...
def test_process_number_token_returns_none_for_unsupported_number():
    """Test that _process_number_token returns None for unsupported numbers."""
    mock_token = Mock()
    mock_token.string = '1e10'
...
def test_run_yields_errors_for_invalid_numbers():
    ...
    assert errors[0][0] == 1
    assert errors[0][1] == 0
    assert 'FDS100' in errors[0][2]


def test_run_skips_non_number_tokens():
...
@patch('flake8_digit_separator.checker.Checker._classify')
def test_process_number_token_returns_error_when_validation_fails(mock_classify, mock_get_validator):
    """Test that _process_number_token returns error when validation fails."""
    mock_number = Mock()
    mock_number.is_supported = True
    mock_number.token = '1234'
...
@patch('flake8_digit_separator.checker.ValidatorRegistry.get_validator')
@patch('flake8_digit_separator.checker.Checker._classify')
def test_process_number_token_returns_none_when_validation_passes(mock_classify, mock_get_validator):
